...
const DEFAULT_APPS = [
    { id: '1', name: '记事本', icon: '📝', iconPath: 'X://HwBOS//app_icon//notepadpp.ico', executable: 'X://HwBOS//built_in_software//Notepad++//notepad++.exe', args: '', type: 'system', width: null, height: null, x: 12, y: 12 },
    { id: '2', name: '计算器', icon: '🧮', iconPath: null, executable: 'calc.exe', args: '', type: 'system', width: null, height: null, x: 12, y: 97 },
    { id: '3', name: '浏览器', icon: '🌐', iconPath: 'X://HwBOS//app_icon//Chrome.png', executable: 'X://HwBOS//built_in_software//Chrome//App//chrome.exe', args: '', type: 'system', width: null, height: null, x: 12, y: 182 },
    { id: '4', name: '文件资源管理器', icon: '📁', iconPath: 'X://HwBOS//app_icon//explorer.ico', executable: 'X://HwBOS//built_in_software//Explorer++//Explorer++64.exe', args: '', type: 'system', width: null, height: null, x: 12, y: 156 },
    { id: '5', name: '7-Zip', icon : '🌐', iconPath: "X://HwBOS//app_icon//7zip.png", executable: 'X://HwBOS//built_in_software//7-Zip//7zFM.exe', args : '', type: 'system', width: null, height: null, x: 12, y: 256 },
    { id : '6' , name : '切换至Windows模式' , icon : '🌐' , iconPath : 'X://HwBOS//app_icon//explorer.ico' , executable : 'X://Windows//System32//desktop.bat' , args : '' , type : 'system' , width : null , height : null , x : 12 , y : 282 },
    { id : '7' , name : 'Word文档' , icon : '🌐' , iconPath : 'X://HwBOS//app_icon//office.png' , executable : 'X://HwBOS//built_in_software//WPS2016//office6//wps.exe' , args : '' , type : 'system' , width : null , height : null , x : 12 , y : 352 },
    { id : '8' , name : '屏幕键盘' , icon : '🌐' , iconPath : 'X://HwBOS//app_icon//freevk.ico' , executable : 'X://HwBOS//built_in_software//FreeVK//osk.exe' , args : '' , type : 'system' , width : null , height : null , x : 12 , y : 382 }
];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 修复 PE 环境 localStorage 兼容（若不可写则用内存存储）
  let storage;
  try {
    ...
    storage.setItem = (key, val) => storage.set(key, val);
  ...
  // 窗口模式切换（兼容 NW.js 所有版本）
  windowedModeBtn.addEventListener('click', () => {
    if (nw) {
  ...
  function initDesktop() {
    ...
    apps.forEach(app => {
      ...
      appItem.style.textAlign = 'center';
      appItem.style.cursor = 'pointer';
      appItem.style.background = '#f5f5f5';
      appItem.style.borderRadius = '4px';
      appItem.innerHTML = `<div style="font-size:24px">${app.icon}</div><div style="font-size:12px;margin-top:5px">${app.name}</div>`;
      // 应用点击逻辑（简化，仅提示）
      appItem.addEventListener('click', () => {
        alert(`PE 环境暂不支持启动「${app.name}」，功能已简化！`);
      });
  ...
  // 修复 PE 环境下的壁纸/背景设置（禁用文件选择，避免权限问题）
  const chooseWallpaper = document.getElementById('chooseWallpaper');
  const clearWallpaper = document.getElementById('clearWallpaper');
  const chooseLoginBg = document.getElementById('chooseLoginBg');
  const clearLoginBg = document.getElementById('clearLoginBg');

  chooseWallpaper.addEventListener('click', () => {
    alert('PE 环境暂不支持选择本地文件，壁纸功能已简化！');
  });
  clearWallpaper.addEventListener('click', () => {
    document.getElementById('wallpaper').style.backgroundImage = 'none';
  });
  chooseLoginBg.addEventListener('click', () => {
    alert('PE 环境暂不支持选择本地文件，登录背景功能已简化！');
  });
  clearLoginBg.addEventListener('click', () => {
    loginOverlay.style.background = 'linear-gradient(135deg, #2c3e50, #4ca1af)';
  });

  // 应用管理简化（PE 环境禁用）
  const addAppBtn = document.getElementById('addAppBtn');
  addAppBtn.addEventListener('click', () => {
    alert('PE 环境暂不支持添加应用，功能已简化！');
  });

  // 电源按钮逻辑（简化）
  const powerBtn = document.getElementById('powerBtn');
  powerBtn.addEventListener('click', () => {
    if (confirm('确定要退出 HwBOS 吗？')) {
      if (nw) nw.Window.get().close();
    }
  });
  ...
  accountBtn.addEventListener('click', () => {
    alert(`当前账户：${storage.getItem('username') || '访客'}\nPE 环境暂不支持账户管理！`);
  });

  // 修复 PE 环境下的窗口层级问题
  desktopContent.style.zIndex = '10';
